...
df=puskesmas
puskesmasMatrix=as.matrix(cbind(df$puskes, df$dokterumum, df$doktergigi, df$perawat, df$bidan),ncol=5)

# execute
fit <- kmeans(puskesmasMatrix, 6)
pus <- ggplot(data=df, aes(x=puskes, y=dokterumum, color = factor(fit$cluster), label= puskesmas$name)) + geom_point() + geom_text_repel(aes(puskes,dokterumum, label= puskesmas$name)) + scale_color_manual(values = c("purple", "black", "blue", "green", "yellow", "gray", "red" ))
